...
T: Dict[str, Dict[str, str]] = {
    UZ: {
        ...
        "sub_no": "Hali obuna bo‘lmagansiz ❌\nKanalga obuna bo‘lib, qayta tekshiring.",
        ...
        "course_intro": (
            "📚 <b>Kursga yozilish</b>\n\n"
            "🎓 Ta’lim: <b>TEKIN</b>\n"
            "💼 Keyin: depozit qilib birga ishlanadi\n"
            "💰 Foyda: <b>50/50</b>\n\n"
        ...
        "partner_text": (
            "🤝 <b>Hamkorlik</b>\n\n"
            "Hamkorlik bo‘yicha yozing:\n"
            f"👉 <a href='{tme(SUPPORT_USERNAME)}'>{SUPPORT_USERNAME}</a>"
        ),
        "results_text": (
            "🏆 <b>Natijalar</b>\n\n"
            "Real natijalar va case’lar kanalda:\n"
            f"👉 <a href='{tme(RESULTS_CHANNEL_USERNAME)}'>{RESULTS_CHANNEL_USERNAME}</a>"
        ),
        "support_text": (
            "👨‍💻 <b>Support</b>\n\n"
            f"👤 {SUPPORT_USERNAME}\n"
    ...
    RU: {
        ...
        "sub_no": "Вы ещё не подписаны ❌\nПодпишитесь и нажмите проверку снова.",
        ...
        "results_text": (
            "🏆 <b>Результаты</b>\n\n"
            "Реальные результаты и кейсы в канале:\n"
            f"👉 <a href='{tme(RESULTS_CHANNEL_USERNAME)}'>{RESULTS_CHANNEL_USERNAME}</a>"
        ),
        "support_text": (
            "👨‍💻 <b>Поддержка</b>\n\n"
...
async def apply_mode(cb: CallbackQuery, state: FSMContext):
    ...
    if key == "off" and OFFICE_MAP_URL:
        await cb.message.answer("📍", reply_markup=kb_office(lang))
    ...
    if ADMIN_ID:
        ...
        admin_msg = (
            ...
            f"📞 Phone: <b>{phone}</b>\n"
            f"📍 Mode: <b>{mode}</b>\n"
            f"🌍 Lang: <b>{'UZ' if lang == 'uz' else 'RU'}</b>\n\n"
            f"🔗 User: {user_link}"
        )
        try:
